...
class Infinite_Scroll_Options {
	...
	function validate( $options ) {
		...
		foreach ( array( 'finishedMsg', 'msgText' ) as $field ) {
			...
			$options['loading'][$field] = stripslashes( addslashes( htmlentities( strip_tags( $options['loading'][$field], '<em><a><strong><p><span>' ) ) ) );
		}
	...
	function get_options( ) {
		...
		$options = Configuration::get( $this->parent->slug_ );
		$options = json_decode($options, true);

		$options = $this->default_options_filter( $options );
		...
		foreach ( array( 'finishedMsg', 'msgText' ) as $field ) {
			...
			$options['loading'][$field] = html_entity_decode( $options['loading'][$field] );
		}
	...
	function default_options_filter( $options ) {
		...
		$options = $this->parse_args( $options,  $this->defaults );
		return $options;
	...
	function set_options( $options, $merge = true ) {
		...
		return Configuration::updateValue( $this->parent->slug_, $options );
